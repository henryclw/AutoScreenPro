...
class AndroidController:
    ...
    def press_menu_key_twice(self):
        ADBPropertiesHelper.run_adb_command(f'adb -s {self.device} shell "input keyevent KEYCODE_MENU && input keyevent KEYCODE_MENU"')  # KEYCODE_MENU=82
    ...
    def swipe(self, start_x: int, start_y: int, end_x: int, end_y: int, duration: int = None):
        if duration is None:
            duration = abs(start_y - start_x) * random.randrange(4, 10)  # too fast would leave some momentum
    ...
    def swipe_up(self, diff_height: int):
        ...
        start_x = random.randrange(abs(diff_width), self.screen_width - abs(diff_width))
